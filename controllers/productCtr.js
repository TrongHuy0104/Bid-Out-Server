    ...
    const productsWithDetails = await Promise.all(
        products.map(async (product) => {
            ...
            return {
                ...
                totalBids: await BiddingProduct.countDocuments({
                    product: product._id,
                }),
            };
        })
...
const updateProduct = asyncHandler(async (req, res) => {
    ...
    product.lengthpic = lengthpic;
    product.width = width;
    product.mediumused = mediumused;
...
// ✅ ADMIN: VERIFY PRODUCT & ADD COMMISSION
const verifyAndAddCommissionProductByAmdin = asyncHandler(async (req, res) => {
    const { commission } = req.body;
    const product = await Product.findById(req.params.id);
    ...
    res.status(200).json({ message: 'Product verified', data: product });
});
